...
def sign_up(k):
    ...
    try:
        ...
        text = requests.get('https://generator.email/').text
        get_user = text.split('onchange="change_username()"')[1].split('class="e7m input-group-addon1 coselect2">')[0].replace('"><div','').replace('value="','').strip()
        get_domain = text.split('onchange="change_write_domain();"')[0].split('coselect3 tt-query s13" autocomplete="off"')[1].split(' id="')[0].replace('value="','').replace('"','').strip()
        email = get_user+"@"+get_domain
        ...
        while True:
            ...
            if n == 8:
                ...
                break
            browser.get(f'https://generator.email/{email}')

            try:
                xpath_el('(//*[text()="Follow this link to verify your email address."]/following-sibling::p)[1]/a')
